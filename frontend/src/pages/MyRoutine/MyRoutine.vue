    ...
    <div class="report-section block">
      ...
      <div class="flex-container expense-container">
        ...
        <ul class="category-list">
          <li
            v-for="(amount, category, index) in expensesByCategory"
            :key="category"
          >
            <span
              :style="{
            ...
            <span class="category-text"
              >{{ category }}: {{ formatCurrency(amount) }}</span
            >
          </li>
        </ul>
      </div>
...
<style scoped>
.container {
  max-width: 1200px; /* 전체 컨테이너 크기 */
  margin: 40px auto; /* Daily Report 아래에 공백 추가 */
  padding: 20px;
...
.graph-right {
  justify-content: flex-end; /* 그래프를 오른쪽으로 이동 */
}
...
.routine-list li {
  display: flex;
  align-items: center;
  margin-bottom: 15px; /* 항목 간의 간격을 넓힘 */
}

.category-text {
  font-size: 1.2em; /* 카테고리 텍스트의 크기를 증가 */
}
