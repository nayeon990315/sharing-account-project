<template>
  <nav class="filter">
    ...
    <div class="otherFilter">
        <form action="#">
            <input type="radio" name='otherFilter' value='recent' @change="handleFilterChange"> 최신 순
            <input type="radio" name='otherFilter' value='manyLikes' @change="handleFilterChange"> 좋아요 순
            <input type="radio" name='otherFilter' value='manyParticipants' @change="handleFilterChange"> 참여자 순
            <input type="radio" name='otherFilter' value='manyCompletions' @change="handleFilterChange"> 달성자 순
            <input type="radio" name='otherFilter' value='myLikes' @change="handleFilterChange"> 나의 좋아요
        </form>
    </div>
...
<div class ="main">
  ...
  <div class="intro">
    ...
    <p>나만의 '벌루틴'을 공유하고, <br>다른 사람들의 '벌루틴'을<br>내 것으로 만들어보세요!</p>
  </div>
  ...
  <div class="cards row row-cols-1 row-cols-md-3 g-4"> 
    ...
    <div class="col" v-for="routine in filteredRoutines" :key="routine.community_id">   

      <div class="card h-100">
        <div class="card-body ">
            <div class="subtitle">
                <span class="type card-subtitle"> {{routine.category_title}} </span>  
                <span class="date card-subtitle"> {{routine.update_date}} </span>  
...
const likesArray = ref([
    ...
    function isLiked(communityId) {
        return likesArray.value.some(like => like.community_id === communityId && like.habit_likes_id === currentId);
    }


    function toggleLike(communityId) {
        const routine = routineCommunityArray.value.find(r => r.community_id === communityId);

        if (isLiked(communityId)) { // 좋아요를 눌러놓은 상태면
            // 좋아요 취소
            const index = likesArray.value.findIndex(
                like => like.community_id === communityId && like.habit_likes_id === currentId
            );
            if (index !== -1) { // index가 있는지 확인차 한번더 체크
            ...
            likesArray.value.push({ community_id: communityId, habit_likes_id: currentId }); // 2. likesArray에 추가
    ...
    function checkMyLike() {
        ...
        const likedRoutines = routineCommunityArray.value.filter(routine => likedCommunityIds.includes(routine.community_id));
    ...
    function applyFilters() {
        ...
        if (selectedCategory.value !== 'all') {
            result = result.filter(routine => routine.category_title === categoryMap[selectedCategory.value]);
        ...
        if (selectedOtherFilter.value === 'myLikes') {
            ...
            result = result.sort((a, b) => new Date(b.update_date) - new Date(a.update_date));
