...
public class RoomInOutController {
    ...
    // 10. 방 현재 구성원 수 증가
    @PutMapping("/increase/member/{roomNum}")
    public ResponseEntity<String> increaseCurrntNumber(@PathVariable int roomNum) {
        try {
            roomInOutService.increaseCurrntNumber(roomNum);
            return ResponseEntity.status(HttpStatus.OK).body("Member count increased successfully.");
        } catch (CustomNotFoundException e) {
            log.info("404 Error : RoomNum not found by {}", roomNum);
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Failed to increase member count.");
        } catch (Exception e) {
            log.info("500 Error : An unexpected error occurred.");
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Failed to increase member count.");
        }
    }

    // 11. 사용자 목록에 정보 추가
    @PostMapping("/add/person")
    public ResponseEntity<String> insertPersonList(@RequestParam int roomNum, @RequestParam String id) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.OK).body("Inserted person list successfully.");
        } catch (CustomNotFoundException e) {
